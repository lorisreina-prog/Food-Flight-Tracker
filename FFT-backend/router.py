...
def _fetch_batch_data(c, batch_id: int) -> dict | None:
    c.execute("SELECT * FROM batch WHERE batch_id=?", (batch_id,))
    batch_row = c.fetchone()
    if not batch_row:
        return None
    data = dict(batch_row)

    c.execute("SELECT within_range FROM cold_chain_log WHERE batch_id=?", (batch_id,))
    ...
    c.execute(
        "SELECT MIN(timestamp), MAX(timestamp) FROM batch_event WHERE batch_id=? AND event_type IN ('arrived','shipped','transported')",
        (batch_id,)
    )
    transport_row = c.fetchone()
    data["transport_min"] = transport_row[0]
    data["transport_max"] = transport_row[1]

    c.execute("SELECT passed FROM quality_check WHERE batch_id=?", (batch_id,))
    qc_rows = c.fetchall()
...
def list_batches(conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    ...
    for b in batches:
        ...
        c.execute("SELECT score FROM trust_score WHERE batch_id=? ORDER BY computed_at DESC LIMIT 1", (bid,))
        ts_row = c.fetchone()
        c.execute("SELECT grade FROM nutri_score WHERE batch_id=? ORDER BY computed_at DESC LIMIT 1", (bid,))
        ns_row = c.fetchone()
...
def get_batch(qr_code: str, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT * FROM batch WHERE qr_code=?", (qr_code,))
    b = c.fetchone()
    if not b:
    ...
    c.execute("SELECT risk_score, risk_level FROM risk_prediction WHERE batch_id=? ORDER BY predicted_at DESC LIMIT 1", (bid,))
    rp_row = c.fetchone()

    c.execute("SELECT AVG(stars) FROM crowd_rating WHERE batch_id=?", (bid,))
    crowd_row = c.fetchone()
    crowd_score = crowd_row[0] if crowd_row and crowd_row[0] is not None else None
    ...
    c.execute("SELECT * FROM recall WHERE batch_id=? ORDER BY issued_at DESC", (bid,))
    recalls = c.fetchall()

    c.execute("SELECT * FROM certificate WHERE batch_id=?", (bid,))
    certs = c.fetchall()
    ...
    c.execute("SELECT co2_total_kg FROM ecological_footprint WHERE batch_id=? ORDER BY computed_at DESC LIMIT 1", (bid,))
    eco_row = c.fetchone()

    c.execute("SELECT within_range FROM cold_chain_log WHERE batch_id=?", (bid,))
    cold_rows = c.fetchall()
    cold_chain_ok = None
    if cold_rows:
...
def create_batch(body: BatchCreate, conn: sqlite3.Connection = Depends(db)):
    import uuid
...
def add_event(batch_id: int, body: EventCreate, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
    c.execute(
    ...
    conn.commit()
    return EventCreated(event_id=c.lastrowid)


@router.post("/api/batch/{batch_id}/recall", response_model=RecallCreated, status_code=201)
def create_recall(batch_id: int, body: RecallCreate, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
    now = now_iso()
    c.execute(
        "INSERT INTO recall (batch_id, reason, severity, issued_at, issued_by) VALUES (?,?,?,?,?)",
        (batch_id, body.reason, body.severity, now, body.issued_by)
    )
    conn.commit()
    recall_id = c.lastrowid
...
def resolve_recall(recall_id: int, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT recall_id FROM recall WHERE recall_id=?", (recall_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="recall not found")
    now = now_iso()
    c.execute("UPDATE recall SET resolved_at=? WHERE recall_id=?", (now, recall_id))
    conn.commit()
    return RecallResolved(recall_id=recall_id, resolved_at=now)


@router.get("/api/recalls/active", response_model=list[ActiveRecall])
def get_active_recalls(conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("""
...
def create_complaint(batch_id: int, body: ComplaintCreate, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
    ...
    c.execute(
        "SELECT COUNT(*) FROM complaint_rate_limit WHERE batch_id=? AND submitted_at > ?",
    ...
    now = now_iso()
    c.execute(
        "INSERT INTO complaint (batch_id, reporter_name, reporter_email, description, category, submitted_at, status) VALUES (?,?,?,?,?,?,'open')",
        (batch_id, body.reporter_name, body.reporter_email, body.description, body.category, now)
    )
    conn.commit()
    complaint_id = c.lastrowid
    c.execute("INSERT INTO complaint_rate_limit (batch_id, submitted_at) VALUES (?,?)", (batch_id, now))
    conn.commit()
...
def list_complaints(conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("""
...
def update_complaint_status(complaint_id: int, body: ComplaintStatusUpdate, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT status FROM complaint WHERE complaint_id=?", (complaint_id,))
    row = c.fetchone()
    if not row:
    ...
    c.execute("UPDATE complaint SET status=? WHERE complaint_id=?", (body.status, complaint_id))
    conn.commit()
    return ComplaintStatusResult(complaint_id=complaint_id, status=body.status)


@router.get("/api/batch/{batch_id}/qr-image")
def get_qr_image(batch_id: int, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT qr_code FROM batch WHERE batch_id=?", (batch_id,))
    row = c.fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="batch not found")
    qr_code = row["qr_code"]
...
def list_stations(conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
...
def create_station(body: StationCreate, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute(
        "INSERT INTO station (name, type, location, operator) VALUES (?,?,?,?)",
        (body.name, body.type, body.location, body.operator)
    )
    conn.commit()
    return StationCreated(station_id=c.lastrowid)


@router.get("/api/batch/{batch_id}/risk", response_model=RiskResponse)
def get_risk(batch_id: int, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
    ...
    one_hour_ago = (datetime.utcnow() - timedelta(hours=1)).strftime("%Y-%m-%dT%H:%M:%S")
    c.execute(
        "SELECT * FROM risk_prediction WHERE batch_id=? AND predicted_at > ? ORDER BY predicted_at DESC LIMIT 1",
        (batch_id, one_hour_ago)
    )
    cached = c.fetchone()
    if cached:
...
def chat(batch_id: int, body: ChatRequest, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    batch_data = _fetch_batch_data(c, batch_id)
    if not batch_data:
        raise HTTPException(status_code=404, detail="batch not found")
    ...
    c.execute(
        "SELECT role, content FROM ai_chat_message WHERE session_id=? AND batch_id=? ORDER BY created_at ASC LIMIT 10",
    ...
    now = now_iso()
    c.execute(
        "INSERT INTO ai_chat_message (session_id, batch_id, role, content, created_at) VALUES (?,?,?,?,?)",
...
def get_alternatives(batch_id: int, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")

    c.execute("SELECT * FROM alternative_product WHERE batch_id=?", (batch_id,))
    rows = c.fetchall()
    if rows:
    ...
    for alt in generated:
        c.execute(
            "INSERT INTO alternative_product (batch_id, product_name, reason, co2_kg, nutri_grade, trust_score) VALUES (?,?,?,?,?,?)",
            (batch_id, alt.get("product_name", ""), alt.get("reason", ""), alt.get("co2_kg"), alt.get("nutri_grade"), alt.get("trust_score"))
        )
        conn.commit()
        c.execute("SELECT * FROM alternative_product WHERE alt_id=?", (c.lastrowid,))
        result.append(AlternativeItem(**dict(c.fetchone())))

    return result


@router.get("/api/batch/{batch_id}/cold-chain", response_model=ColdChainResponse)
def get_cold_chain(batch_id: int, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
...
def add_cold_chain_log(batch_id: int, body: ColdChainLogCreate, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
    ...
    now = now_iso()
    c.execute(
        "INSERT INTO cold_chain_log (batch_id, station_id, recorded_at, temp_celsius, min_temp, max_temp, within_range) VALUES (?,?,?,?,?,?,?)",
        (batch_id, body.station_id, now, body.temp_celsius, body.min_temp, body.max_temp, within_range)
    )
    conn.commit()
    return ColdChainLogCreated(log_id=c.lastrowid, within_range=bool(within_range))


@router.get("/api/batch/{batch_id}/iot/live", response_model=IoTLiveResponse)
def get_iot_live(batch_id: int, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
...
def add_iot_reading(batch_id: int, body: IoTReadingCreate, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT product_category FROM batch WHERE batch_id=?", (batch_id,))
    batch_row = c.fetchone()
    if not batch_row:
        raise HTTPException(status_code=404, detail="batch not found")

    c.execute("SELECT station_id FROM station WHERE station_id=?", (body.station_id,))
    if not c.fetchone():
    ...
    if body.sensor_type == "temperature":
        ...
        if not (min_t <= body.value <= max_t):
            c.execute(
                "INSERT INTO cold_chain_log (batch_id, station_id, recorded_at, temp_celsius, min_temp, max_temp, within_range) VALUES (?,?,?,?,?,?,0)",
                (batch_id, body.station_id, now, body.value, min_t, max_t)
            )
            conn.commit()

            c.execute(
                "SELECT value, recorded_at FROM iot_reading WHERE batch_id=? AND sensor_type='temperature' ORDER BY recorded_at DESC LIMIT 5",
                (batch_id,)
            )
    ...
    c.execute(
        "INSERT INTO iot_reading (batch_id, station_id, sensor_type, value, recorded_at, notes) VALUES (?,?,?,?,?,?)",
        (batch_id, body.station_id, body.sensor_type, body.value, now, anomaly_notes)
    )
    conn.commit()
    return IoTReadingCreated(reading_id=c.lastrowid)


@router.post("/api/batch/{batch_id}/iot/simulate", response_model=IoTSimulateResponse, status_code=201)
def simulate_iot(batch_id: int, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT product_category FROM batch WHERE batch_id=?", (batch_id,))
    batch_row = c.fetchone()
    if not batch_row:
    ...
    c.execute(
        "SELECT recall_id FROM recall WHERE batch_id=? AND resolved_at IS NULL AND severity='critical' LIMIT 1",
        (batch_id,)
    )
...
def get_iot_anomalies(batch_id: int, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
    c.execute("""
        ...
        FROM iot_reading ir
        JOIN station s ON ir.station_id = s.station_id
        WHERE ir.batch_id=? AND ir.notes IS NOT NULL
        ORDER BY ir.recorded_at DESC
    """, (batch_id,))
...
def get_nutri_score(batch_id: int, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT * FROM nutri_score WHERE batch_id=? ORDER BY computed_at DESC LIMIT 1", (batch_id,))
    row = c.fetchone()
    if not row:
...
def add_nutri_score(batch_id: int, body: NutriScoreCreate, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
...
def get_eco_footprint(batch_id: int, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT * FROM ecological_footprint WHERE batch_id=? ORDER BY computed_at DESC LIMIT 1", (batch_id,))
    row = c.fetchone()
    if not row:
...
def add_eco_footprint(batch_id: int, body: EcoFootprintCreate, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
    ...
    conn.commit()
    return EcoFootprintCreated(eco_id=c.lastrowid)


@router.get("/api/batch/{batch_id}/price-breakdown", response_model=list[PriceBreakdownItem])
def get_price_breakdown(batch_id: int, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT * FROM price_breakdown WHERE batch_id=?", (batch_id,))
    rows = c.fetchall()
    return sorted(
...
def add_price_breakdown(batch_id: int, body: PriceBreakdownCreate, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
    c.execute(
        "INSERT INTO price_breakdown (batch_id, stage, cost_chf, margin_pct, notes) VALUES (?,?,?,?,?)",
        (batch_id, body.stage, body.cost_chf, body.margin_pct, body.notes)
    )
    conn.commit()
    return PriceBreakdownCreated(price_id=c.lastrowid)


@router.get("/api/batch/{batch_id}/trust-score", response_model=TrustScoreResponse)
def get_trust_score(batch_id: int, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
    score = compute_trust_score(conn, batch_id)
    c.execute("SELECT factors FROM trust_score WHERE batch_id=? ORDER BY computed_at DESC LIMIT 1", (batch_id,))
    row = c.fetchone()
...
def get_leaderboard(conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
...
def add_crowd_rating(batch_id: int, body: CrowdRatingCreate, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE batch_id=?", (batch_id,))
    if not c.fetchone():
        raise HTTPException(status_code=404, detail="batch not found")
    c.execute("SELECT rating_id FROM crowd_rating WHERE batch_id=? AND user_token=?", (batch_id, body.user_token))
    if c.fetchone():
        raise HTTPException(status_code=400, detail="user already rated this batch")
    now = now_iso()
    c.execute(
        "INSERT INTO crowd_rating (batch_id, stars, comment, user_token, submitted_at) VALUES (?,?,?,?,?)",
...
def get_achievements(user_token: str, conn: sqlite3.Connection = Depends(db)):
    c = conn.cursor()
    c.execute(
        "SELECT achievement_type, earned_at, batch_id FROM achievement WHERE user_token=? ORDER BY earned_at DESC",
        (user_token,)
    )
...
def scan_ocr(body: OcrRequest, conn: sqlite3.Connection = Depends(db)):
    ...
    c = conn.cursor()
    c.execute("SELECT batch_id FROM batch WHERE qr_code=?", (qr_code,))
    row = c.fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="no batch found in image")
